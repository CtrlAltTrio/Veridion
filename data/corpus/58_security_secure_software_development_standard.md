...
This standard defines required controls from design through production release. It is the controlled internal source for this topic. Northwind Systems uses it to make decisions consistently across Portland, Austin, Dublin, and remote teams. Readers should check the owner and review date before acting, particularly when a customer contract, regional rule, or active incident may affect the normal workflow.

## Authoritative facts

- Every production change requires review by a second engineer.
- Secret scanning runs on every pull request.
- Threat modeling is required for new external authentication flows.
- Critical code-scanning findings block release.

...
This runbook protects customers during sensitive account and billing changes. It is the controlled internal source for this topic. Northwind Systems uses it to make decisions consistently across Portland, Austin, Dublin, and remote teams. Readers should check the owner and review date before acting, particularly when a customer contract, regional rule, or active incident may affect the normal workflow.

## Authoritative facts

- Support verifies the requester through an authenticated workspace session.
- Domain ownership changes require confirmation from two workspace owners.
- Support never asks for a password or MFA recovery code.
- Failed verification is recorded and escalated to Trust Support.

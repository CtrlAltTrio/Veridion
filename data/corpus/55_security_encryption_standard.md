...
This standard sets approved encryption controls for customer and corporate data. It is the controlled internal source for this topic. Northwind Systems uses it to make decisions consistently across Portland, Austin, Dublin, and remote teams. Readers should check the owner and review date before acting, particularly when a customer contract, regional rule, or active incident may affect the normal workflow.

## Authoritative facts

- Customer data at rest uses AES-256 encryption.
- External network connections require TLS 1.2 or later.
- Private keys are rotated at least every 12 months.
- Encryption keys are stored separately from encrypted application data.
